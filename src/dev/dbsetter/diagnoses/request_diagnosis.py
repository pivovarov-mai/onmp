...
mkb.name as "Код МКБ", diagnosis.name_diagnosis AS "Диагноз", sub_diagnosis.name_sub_diagnosis AS "Поддиагноз",
omp.name_omp AS "Объем медицинской помощи", sub_diagnosis.recommendations AS "Рекомендации поддиагноза"
FROM diagnosis
JOIN mkb_diagnosis ON diagnosis.id = mkb_diagnosis.diagnosis_id
JOIN mkb ON mkb.id = mkb_diagnosis.mkb_id
LEFT JOIN sub_diag_diagnosis ON diagnosis.id = sub_diag_diagnosis.diagnosis_id
